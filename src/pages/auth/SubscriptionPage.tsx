...
export const SubscriptionPage: React.FC = () => {
  ...
  const handleContinue = async (e: React.FormEvent) => {
    ...
    navigate('/subscription/otp', { replace: true });
  };
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F6] text-slate-900 flex flex-col font-sans">
      <SEOHead
        ...
        path="/subscription"
        title="Subscription | SkillProof"
        description="Subscribe to SkillProof to access all career and skill verification features."
        ...
        <div className="w-full max-w-md">
          <div className="text-center mb-7">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Confirm your BDApps subscription to unlock the full SkillProof dashboard.
            </p>
          </div>
          ...
          <div className="relative">
            ...
            <BrandCard elevation="raised" accent className="relative">
              ...
              <div className="mt-5 grid grid-cols-3 gap-2 text-center">
                <BenefitCap label="Real skills" />
                <BenefitCap label="Verified" />
                <BenefitCap label="Career fast" />
              </div>
...
/**
 * Premium subscription-info card. Sits above the mobile-number input so
 * users see the price and the supported operators before they commit.
 *
 * - Subtle red-tinted background + thin brand border (premium feel).
 * - "2.78 BDT/day" rendered in the primary brand colour at a larger
 *   weight than the surrounding text, so the eye lands on it first.
 * - "Robi & Airtel Only" rendered as a pill chip with a smartphone icon.
 * - Fully responsive: stacks on mobile, side-by-side from `sm` upward.
 * - UI-only; no state, no callbacks, no business logic.
 */
const SubscriptionInfoCard: React.FC = () => (
  ...
  >
    {/* subtle decorative sheen, pointer-events disabled so it never intercepts clicks */}
    <span
      aria-hidden
    ...
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 text-sm font-semibold leading-snug text-emerald-900">
          You already have full SkillProof access — no BDApps subscription needed.
        </p>
        {premiumUntil && (
    ...
    <div className="rounded-2xl border border-emerald-200/80 bg-gradient-to-br from-emerald-50 via-white to-emerald-50/60 p-5">
      ...
      <ul className="mt-4 space-y-2 text-[13px] text-slate-700">
        <li className="flex items-start gap-2">
          ...
          Full dashboard and skill workflows unlocked
        </li>
        <li className="flex items-start gap-2">
          <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />
