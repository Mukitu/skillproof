 ...
 * User Roadmap Center.
 *
 * Three sub-views (URLs):
 *   /dashboard/roadmap                                  -> library + enrollments
 *   /dashboard/roadmap/:enrollmentId                    -> day list (locked view)
 *   /dashboard/roadmap/:enrollmentId/day/:dayNumber      -> day details
 *
 * Locked days only show day_number, title and a lock icon. Description,
 * materials, tasks etc. are loaded only when the user opens an unlocked day.
 * 24-hour unlock rule is enforced server-side by fn_user_complete_roadmap_day.
 */
import { useCallback, useEffect, useMemo, useState } from 'react';
import type { Key as ReactKey } from 'react';
import { Link, useNavigate, useParams } from 'react-router-dom';
import {
  ArrowLeft, BookOpen, Calendar, CheckCircle2, ChevronRight, Clock,
  Filter, Library, Link2, ListOrdered, Loader2, Lock, Play, Search,
  Sparkles, Target, TrendingUp, Unlock, Youtube, Award, GraduationCap,
  ListChecks, NotebookPen, ExternalLink,
} from 'lucide-react';
import {
  completeEnrollmentDay, enrollInRoadmap, getEnrollmentModules,
  getEnrollmentProgress, getRoadmapEnrollment, getRoadmapTemplate,
  getRoadmapTemplateDay, getUnlockedDayDetails, listMyRoadmapEnrollments,
...
import type {
  ...
  Category, RoadmapTemplate, RoadmapTemplateDay, SubCategory,
} from '../../types/database';
...
function LibraryAndMyRoadmaps({ onOpenEnrollment }: { onOpenEnrollment: (id: string) => void }) {
  ...
  useRealtimeRefresh(
    ['roadmap_templates', 'career_roadmap_enrollment', 'career_roadmap_progress', 'career_roadmap_modules'],
    reload,
  );
  ...
  return (
    <div className="space-y-8">
      {/* Hero */}
      <div className="overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 p-6 text-white shadow">
        ...
        <h1 className="mt-2 text-3xl font-bold">ক্যারিয়ার রোডম্যাপ লাইব্রেরি</h1>
        <p className="mt-1 max-w-2xl text-sm opacity-90">একাধিক রোডম্যাপে একসাথে enroll করুন। প্রতিটি দিন আগের দিন সম্পন্ন হওয়ার ২৪ ঘণ্টা পরে খোলে — সার্ভার-সাইড enforced।</p>
      </div>
      ...
      <section>
        ...
        {enrollments.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {enrollments.map((e) => <EnrollmentCard key={e.id} enrollment={e} onOpen={() => onOpenEnrollment(e.id)} />)}
          </div>
        )}
      </section>
      ...
      <section>
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="relative flex-1 min-w-[200px]"><Search size={16} className="absolute top-1/2 left-3 -translate-y-1/2 text-slate-400" /><input type="text" placeholder="রোডম্যাপ খুঁজুন..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full rounded-lg border border-slate-200 bg-slate-50 py-2 pr-3 pl-9 text-sm" /></div>
          <div className="flex items-center gap-1 text-xs text-slate-500"><Filter size={12} /> ফিল্টার:</div>
          <select value={categoryFilter} onChange={(e) => { setCategoryFilter(e.target.value); setSubFilter(''); }} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"><option value="">সকল ক্যাটাগরি</option>{categories.filter((c) => c.status === 'Active').map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
          <select value={subFilter} onChange={(e) => setSubFilter(e.target.value)} disabled={!categoryFilter} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm disabled:opacity-50"><option value="">সকল সাব-ক্যাটাগরি</option>{filteredSubs.filter((s) => s.status === 'Active').map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select>
        </div>
...
function EnrollmentCard({ enrollment: e, onOpen }: { enrollment: CareerRoadmapEnrollment; onOpen: () => void; key?: ReactKey }) {
  ...
  return (
    <div onClick={onOpen} className="group cursor-pointer overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:shadow-md">
      ...
      <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-indigo-600" style={{ width: `${progressPct}%` }} /></div>
    </div>
  );
}
...
function TemplateCard({ template: t, categories, subCategories, enrolled, enrolling, onEnroll }: { template: RoadmapTemplate; categories: Category[]; subCategories: SubCategory[]; enrolled: boolean; enrolling: boolean; onEnroll: () => Promise<void>; key?: ReactKey }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:shadow-md">
      <div className="flex h-36 items-center justify-center overflow-hidden bg-gradient-to-br from-blue-500 to-indigo-600">
      ...
      <div className="space-y-2 p-4">
        ...
        <button onClick={onEnroll} disabled={enrolled || enrolling} className="mt-2 flex w-full items-center justify-center gap-1 rounded-lg bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-40">
...
function EnrollmentDetailRoute({ enrollmentId }: { enrollmentId: string }) {
  ...
  const reload = useCallback(async () => {
    try {
      ...
      const [m, p] = await Promise.all([getEnrollmentModules(enrollmentId), getEnrollmentProgress(enrollmentId)]);
      setModules(m); setProgress(p); setError('');
    } catch (e: any) { setError(e.message); }
  }, [enrollmentId]);

  useEffect(() => { void reload(); }, [reload]);
  useRealtimeRefresh(['career_roadmap_modules', 'career_roadmap_progress', 'career_roadmap_enrollment'], reload);
  ...
  const isUnlocked = (n: number) => {
    ...
    return !!prev?.completed_at && new Date(prev.completed_at).getTime() + 24 * 60 * 60 * 1000 <= now;
  };
        ...
        {modules.length === 0 ? (
          ...
          return (
            <button
              ...
              disabled={!unlocked}
              className={`flex w-full items-center gap-4 rounded-xl border bg-white p-3 text-left transition ${
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <h3 className="mt-0.5 text-sm font-semibold text-slate-900">{m.title}</h3>
              </div>
              <div className="flex items-center gap-2">
                {unlocked && <span className="flex items-center gap-1 text-xs text-slate-500"><Clock size={12} /> {m.estimated_minutes} মিনিট</span>}
...
function DayDetailRoute({ enrollmentId, dayNumber }: { enrollmentId: string; dayNumber: number }) {
  ...
  useEffect(() => { void reload(); }, [reload]);
  useRealtimeRefresh(['career_roadmap_progress', 'roadmap_template_days'], reload);
  ...
  const embedUrl = buildEmbedUrl(day.video_url, day.video_provider);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      {embedUrl && (
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          {day.video_title && <div className="flex items-center gap-2 border-b border-slate-200 px-5 py-3 text-sm font-semibold text-slate-800"><Youtube size={16} className="text-red-500" /> {day.video_title}</div>}
      ...
      <div className="sticky bottom-4 z-10 mx-auto flex max-w-md items-center justify-center gap-3 rounded-full border border-slate-200 bg-white p-3 shadow-lg">
        {completed ? (
          <div className="flex items-center gap-2 text-sm text-emerald-700"><CheckCircle2 size={18} /> সম্পন্ন হয়েছে — {new Date(completed.completed_at!).toLocaleString('bn-BD')}</div>
        ) : (
...
function buildEmbedUrl(url: string | null | undefined, provider: string | null | undefined): string | null {
  if (!url) return null;
  if (provider === 'embed') return url;
  if (provider === 'youtube' || /youtu\.?be/i.test(url)) {
    try {
      const u = new URL(url);
