...
/**
 * Company-side mirror of `SubscriptionContext`.
 *
 * The user BDApps flow (`SubscriptionContext`) is keyed off `profile.user_id`
 * and uses the canonical `supabase` client. Companies live on a different
 * Supabase client (`companySupabase`) with a separate localStorage key, so
 * they can't share that context — they need the same logic but bound to the
 * company owner's `auth.users.id` and the company client.
 *
 * The BDApps *service* (`bdapps.ts`) is operator-level and is identical for
 * both portals, so we reuse it as-is. The realtime channels are renamed
 * (`skillproof:company-subscription:...`) so a company subscription event
 * never bleeds into the user portal's realtime stream.
 *
 * Admin-granted premium is the same as the user side: when the company row
 * has `premium_until > now()`, `isPremiumActive` is true and the Company
 * subscription page short-circuits to the dashboard.
 */

export interface CompanySubscriptionSession {
  phone: string;
  subscriberId: string;
  status: BdappsStatus | '';
  verifiedAt: string;
...
interface CompanySubscriptionContextValue {
  session: CompanySubscriptionSession | null;
  ...
  verifyStatus: (phone: string) => Promise<
    { ok: true; subscribed: true } | { ok: true; subscribed: false } | { ok: false; error: string }
  >;

  confirmOtp: (otp: string) => Promise<{ ok: true } | { ok: false; error: string }>;
...
export function isCompanyAdminPremiumActive(
...
const CompanySubscriptionContext = createContext<CompanySubscriptionContextValue | undefined>(undefined);
...
function storageKey(userId: string): string {
  return `skillproof:bdapps:company-subscription:${userId}`;
}

function safeReadSession(userId: string | null): CompanySubscriptionSession | null {
  if (!userId) return null;
  if (typeof window === 'undefined') return null;
  try {
    const raw = window.localStorage.getItem(storageKey(userId));
    if (!raw) return null;
    const parsed = JSON.parse(raw) as CompanySubscriptionSession;
...
function safeWriteSession(userId: string | null, value: CompanySubscriptionSession | null): void {
...
export const CompanySubscriptionProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  const { company } = useCompanyAuth();
  const userId = company?.user_id ?? null;

  const [session, setSession] = useState<CompanySubscriptionSession | null>(null);
  ...
  const broadcastInvalidation = useCallback(
    (reason: string, fromStatus: string | null) => {
      ...
      // Same-tab BroadcastChannel + cross-tab localStorage fallback as the
      // user side, but namespaced to the company flow so they don't
      // interfere.
      try {
        const ch = new BroadcastChannel('skillproof:bdapps:company-subscription');
        ch.postMessage({
      ...
      try {
        if (typeof window !== 'undefined' && userId) {
          window.localStorage.setItem(
            `skillproof:bdapps:company-invalidate:${userId}`,
  ...
  // Cross-tab / cross-context invalidation listener
  useEffect(() => {
    if (!userId) return;
    ...
    try {
      bc = new BroadcastChannel('skillproof:bdapps:company-subscription');
    ...
    const onStorage = (e: StorageEvent) => {
      ...
      if (e.key !== `skillproof:bdapps:company-invalidate:${userId}`) return;
      try {
  ...
  // Realtime self-broadcast channel
  useEffect(() => {
    if (!userId) return;
    let channel: ReturnType<typeof companySupabase.channel> | null = null;
    try {
      channel = companySupabase
        .channel(`skillproof:company-subscription:${userId}`, {
          config: {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      if (cancelled) return;
      try {
        // Probe the company DB first — if our row says we're not subscribed,
        // we don't need to spend an API call on BDApps.
        try {
          const { data: latestRow, error: latestErr } = await companySupabase
            .from('bdapps_company_subscriptions' as any)
            .select('status')
            .eq('user_id', userId)
          ...
          if (
            !cancelled &&
            !latestErr &&
            (latestRow as { status?: string } | null)?.status &&
            !isEntitledStatus((latestRow as { status?: string }).status)
          ) {
            const latestStatus = (latestRow as { status: string }).status;
          ...
          // ignore — fall back to BDApps check
        }
        ...
        if (!stillEntitled) {
          ...
          broadcastInvalidation(resp.subscriptionStatus || 'unknown', fromStatus);
          try {
            await companySupabase
              .channel(`skillproof:company-subscription:${userId}`)
              .send({
        ...
        // Network / BDApps blip — fall back to cache-age guard.
  ...
  // Postgres-changes listener on the company subscription table
  useEffect(() => {
    if (!userId || !session?.subscriberId) return;
    let channel: ReturnType<typeof companySupabase.channel> | null = null;
    try {
      channel = companySupabase
        .channel(`bdapps-company-subs:${userId}:${Math.random().toString(36).slice(2)}`)
        .on(
          'postgres_changes' as any,
          {
            event: '*',
            schema: 'public',
            table: 'bdapps_company_subscriptions' as any,
  ...
  const requestOtpForPhone = useCallback(
    async (
      phone: string,
      subscriberId: string,
  ...
  const persist = useCallback(
    (next: CompanySubscriptionSession | null) => {
      setSession(next);
  ...
  > => {
    ...
    try {
      ...
      persist(null);
      setPendingOtp(null);
      broadcastInvalidation(resp.subscriptionStatus || 'UNREGISTERED', fromStatus);
      try {
        await companySupabase
          .channel(`skillproof:company-subscription:${userId}`)
          .send({
  ...
  }, [persist, session, broadcastInvalidation, userId]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const resp = await checkSubscription(session.phone);
      const next: CompanySubscriptionSession = {
        phone: session.phone,
        subscriberId: resp.subscriberId || session.subscriberId,
  ...
  const verifyStatus = useCallback<CompanySubscriptionContextValue['verifyStatus']>(
    async (phone) => {
      setIsLoading(true);
      setError(null);
      try {
  ...
  const confirmOtp = useCallback<CompanySubscriptionContextValue['confirmOtp']>(
    async (otp) => {
      if (!pendingOtp) {
  ...
  const resendOtp = useCallback<CompanySubscriptionContextValue['resendOtp']>(async () => {
    if (!pendingOtp) {
  ...
  const premiumState = isCompanyAdminPremiumActive(company?.premium_until);
  const isPremiumActive = premiumState.active;
  const premiumUntil = premiumState.until ? premiumState.until.toISOString() : null;

  const value = useMemo<CompanySubscriptionContextValue>(
    () => ({
      session,
...
export function useCompanySubscription(): CompanySubscriptionContextValue {
  const ctx = useContext(CompanySubscriptionContext);
  if (!ctx) {
    throw new Error('useCompanySubscription must be used within a CompanySubscriptionProvider');
  }
  return ctx;
}

export const COMPANY_SUBSCRIPTION_PUBLIC_CONSTANTS = BDAPPS_CONSTANTS;
