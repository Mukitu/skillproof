-- Migration 12 - Passport trigger: 5 passed submissions for the same skill => pending_approval passport.
...
DECLARE
  ...
  v_public_id TEXT;
BEGIN
  -- Only act on transitions to Passed.
    ...
    IF passed_count >= 5 THEN
      ...
      IF v_existing_passport IS NULL THEN
        ...
        INSERT INTO public.skill_passports (
          ...
          'pending_approval', v_skill_name || ' Verified Expert', false,
        ...
        VALUES (
          ...
          'You have completed 5 distinct ' || v_skill_name || ' assessments. Your Skill Passport is pending admin verification.',
          'passport_upgrade',
          '/dashboard/passport'
        );
      END IF;
...
-- Also handle INSERT case where status is set to Passed directly.
