...
content = content.replace(/\{!isGenerating && !generatedAssessment && \((.|\n)*?\{language === 'bn' \? 'অ্যাসেসমেন্ট জেনারেট করুন' : 'Generate Technical Assessment'\}\n\s*?<\/button>\n\s*?<\/div>\n\s*?\)\}/g, `{!isGenerating && !generatedAssessment && (
            ...
            {!isGenerating && activeAssessment && (
              <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-sm font-bold text-slate-900">Active Assessment Restored</h4>
                  <p className="text-xs text-slate-600 mt-1">
                    You have an incomplete assessment for this skill. You must submit a solution for it before generating a new one.
                  </p>
                </div>
              </div>
            )}`);
